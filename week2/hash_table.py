...
class HashTable:
    ...
    def rehash(self):
        if self.item_count / self.bucket_size > 0.7:
            self.double_tablesize()
        if self.item_count / self.bucket_size < 0.3:
            self.halve_tablesize()
    ...
    def put(self, key, value):
        ...
        while item:
            if item.key == key:  # if the item is found
                item.value = value  # update the value
                return False
            item = item.next
    ...
    def delete(self, key):
        ...
        while item:
            if item.key == key:
                if prev:  # if the item is not the first item in the linked list
                    prev.next = item.next
                else:  # if the item is the first item in the linked list
