...
from calculator_class import Tokenizer, Validator, Evaluator, Calculator
...
def test_validate_ok(input_ok):
  tokens = Tokenizer(input_ok).tokenize()
  assert Validator(tokens).validate() is not None

@pytest.mark.parametrize("input_ng", [
  "-", "+", "*", "/", "(", ")", "abs -1", "int2.9999", "round9.9999", "absint", "roundabs", "roundabsint****/////((()))))))", "roundabs+int-*1)/(123"
])
def test_validate_ng(input_ng):
  tokens = Tokenizer(input_ng).tokenize()
  assert Validator(tokens).validate() is None
...
def test_calculate_ok(input):
  assert Calculator(input).calculate() - eval(input) < 1e-8
